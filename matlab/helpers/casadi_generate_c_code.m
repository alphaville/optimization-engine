...
nu = length(u); np = length(p); ncp = length(cstr_penalty);
...
try, movefile(cost_c_file, cost_c_path); end

jphi_fun.generate(grad_c_file);
try, movefile(grad_c_file, grad_c_path); end

cstr_penalty_fun.generate(constr_penalty_c_file);
try, movefile(constr_penalty_c_file, constr_penalty_c_file_path); end
