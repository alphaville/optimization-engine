...
Here, `x_ref`, `q`, and `r` are parameters. The terminal cost function is:
...
Lastly, we have the state constraint `x_t >= x_min`, where `x_min` is a parameter, and the hard input constraints `|u_t| <= 0.2`.
