function [phi_fun, jphi_fun] = casadi_generate_c_code(u, p, phi, function_name )
...
if nargin < 4, function_name = 'phi'; end

nu = length(u); np = length(p);

rd = which('casadi_generate_c_code');
tokens = strsplit(rd, 'matlab/casadi_generate_c_code.m');
rd = tokens{1};
...
cost_c_path = fullfile(rd, 'extern', cost_c_file);
grad_c_path = fullfile(rd, 'extern', grad_c_file);
...
% -------------------------------------------------------------------------
casadi_interface_destination = fullfile(rd, 'extern', 'icasadi_config.h');
